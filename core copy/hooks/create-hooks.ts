 ...
 */
export function useHook(render: any, layoutEffect?: any, effect?: any) {
    let { i, hooks } = SCOPE;
 ...
 * @returns {{current:T}}
 */
export let useRef = (current: any) => useHook((ref = { current }) => ref);
 ...
 */
export function createHooks(update: () => void, host: any) {
    /**
     ...
     */
    function cleanEffectsType(type: number, unmounted: any) {
    ...
    function load(callback: () => any) {
        let SCOPE: any = { host, hooks, update, i: 0 };
        let value;
        try {
            value = callback();
        } finally {
     ...
     * @returns {()=>void}
     */
    function cleanEffects(unmounted: any) {
