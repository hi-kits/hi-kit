 ...
 **/
let TRUE_VALUES: {[x: string]: any;} = { true: 1, "": 1, 1: 1 };
 ...
 */
export function setPrototype(prototype: Object, prop: string, schema: any, attrs: any, values: { [x: string]: any; }) {
    /**@type {Schema} */
    let {
        type,
        reflect,
        ...
        value,
        attr = getAttr(prop),
    }: any = isObject(schema) && schema != Any ? schema : { type: schema };
 ...
 */
export let dispatchEvent = (node:any, { type, base = CustomEvent, ...eventInit }: any) =>
 ...
 * @returns {string}
 */
export let getAttr = (prop: string): string => prop.replace(/([A-Z])/g, "-$1").toLowerCase();
 ...
 * @param {any} value
 */
export let reflectValue = (host: Element, type: any, attr: string, value: any) =>
 ...
 * @returns {any}
 */
export let transformValue = (type: any, value: string): any =>
    type == Boolean
        ? !!TRUE_VALUES[value]
        : type == Number
        ? Number(value)
 ...
 */
export let filterValue = (type: any, value: any): { error?: boolean; value: any; } =>
